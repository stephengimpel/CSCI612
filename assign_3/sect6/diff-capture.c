...
static void process_frame(unsigned char *img, int width, int height, int frame_id, unsigned char *prev) {
    struct timespec t;
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            int index = i * width + j;
            unsigned char cur_pixel = img[index];
            unsigned char prev_pixel = prev[index];

            int diff = (int)cur_pixel - (int)prev_pixel;
...
static void process_input_directory(const char *dirpath) {
    ...
    for (size_t i = 1; i < count; i++) {
        ...
        unsigned char *prev = read_pgm(files[i - 1], &w, &h);
        if (!img) continue;

        printf("Processing %s (%dx%d)\n", files[i], w, h);

        process_frame(img, w, h, i, prev);
