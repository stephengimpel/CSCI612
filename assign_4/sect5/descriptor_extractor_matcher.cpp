...
void help(char **argv) {
    cout << "\nThis program demonstrates keypoint finding and matching between 2 images using features2d framework.\n"
         ...
         << "Example of case1:\n"
         << "./descriptor_extractor_matcher SIFT SIFT FlannBased NoneFilter cola.jpg 0\n"
         << "\n"
         ...
         << "Example of case2:\n"
         << "./descriptor_extractor_matcher SIFT SIFT BruteForce CrossCheckFilter cola1.jpg cola2.jpg 3\n"
         << "\n"
...
void simpleMatching(Ptr<DescriptorMatcher> &descriptorMatcher, const Mat &descriptors1, const Mat &descriptors2, vector<DMatch> &matches12) {
...
void doIteration(const Mat &img1, Mat &img2, bool isWarpPerspective, vector<KeyPoint> &keypoints1, const Mat &descriptors1, Ptr<Feature2D> &detector, Ptr<Feature2D> &descriptorExtractor, Ptr<DescriptorMatcher> &descriptorMatcher, int matcherFilter, bool eval, double ransacReprojThreshold, RNG &rng) {
    assert(!img1.empty());
    Mat H12;
    ...
    cout << endl
         << "< Extracting keypoints from second image..." << endl;
    ...
    if (!H12.empty() && eval) {
        // Simple repeatability approximation: count keypoints in overlapping region
        cout << "< Evaluating feature detector (keypoint counts)..." << endl;
    ...
    if (!H12.empty() && eval) {
        // Simplified evaluation: count inlier matches using the homography
        cout << "< Evaluating descriptor matcher (inlier count)..." << endl;
        int inlierCount = 0;
        ...
        for (const auto &m : filteredMatches) {
            ...
            vector<Point2f> p1vec = {p1}, p1t;
            perspectiveTransform(p1vec, p1t, H12);
            if (norm(p2 - p1t[0]) <= inlierThreshold)
                inlierCount++;
        }
        cout << "inlier matches = " << inlierCount << " / " << filteredMatches.size() << endl;
        cout << ">" << endl;
    }
    ...
    if (!H12.empty()) {
        ...
        drawMatches(img1, keypoints1, img2, keypoints2, filteredMatches, drawImg, Scalar(0, 0, 255), Scalar(255, 0, 0), matchesMask, DrawMatchesFlags::DRAW_OVER_OUTIMG | DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS);
#endif
    } else {
...
int main(int argc, char **argv) {
    ...
    Ptr<Feature2D> descriptorExtractor = SIFT::create();
    ...
    if (detector.empty() || descriptorExtractor.empty() || descriptorMatcher.empty()) {
        cout << "Cannot create detector or descriptor extractor or descriptor matcher of given types" << endl;
        return -1;
    }
    ...
    cout << endl
         << "< Extracting keypoints from first image..." << endl;
    ...
    RNG rng = theRNG();
    doIteration(img1, img2, isWarpPerspective, keypoints1, descriptors1, detector, descriptorExtractor, descriptorMatcher, matcherFilterType, eval, ransacReprojThreshold, rng);
