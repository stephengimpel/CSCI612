...
int main() {
    ...
    while (true) {
        ...
        dilate(skel, skel, getStructuringElement(MORPH_ELLIPSE, Size(3, 3)));

        findContours(skel.clone(), contours, hierarchy, RETR_EXTERNAL, CHAIN_APPROX_SIMPLE);
        ...
        if (!contours.empty()) {
            ...
            if (bestIdx >= 0 && bestArea > 300) {
                vector<Point> handContour = contours[bestIdx];

                // Smooth contour a bit
                vector<Point> approxContour = handContour;
                ...
                convexHull(approxContour, hullIndices, false);

                vector<Point> hullPoints;
                convexHull(approxContour, hullPoints, false);
                polylines(roi, hullPoints, true, Scalar(0, 255, 0), 2);

                vector<Vec4i> defects;
                if (hullIndices.size() >= 3 && approxContour.size() >= 4) {
                ...
                for (const auto &d : defects) {
                    ...
                    bool validDepth = depth > handBox.height * 0.03;
                    bool validAngle = ang < 110.0;
                    bool startHigh = start.y < palmYThreshold;
                    bool endHigh = end.y < palmYThreshold;
                    bool farBelowTips = (far.y > start.y || far.y > end.y);
                    bool startFarEnough = startDist > handBox.height * 0.2;
                    bool endFarEnough = endDist > handBox.height * 0.2;
                    bool valleyReasonable = farDist > handBox.height * 0.08;
                ...
                putText(roi, "Fingers: " + to_string(fingerCount), Point(10, 30), FONT_HERSHEY_SIMPLEX, 1.0, Scalar(0, 255, 0), 2);
            } else {
                putText(roi, "Show hand in box", Point(10, 30), FONT_HERSHEY_SIMPLEX, 0.8, Scalar(0, 0, 255), 2);
            }
        } else {
            putText(roi, "No hand found", Point(10, 30), FONT_HERSHEY_SIMPLEX, 0.8, Scalar(0, 0, 255), 2);
        }
        ...
        filename << "output/finger_" << setw(6) << setfill('0') << framecount++ << +".png";
